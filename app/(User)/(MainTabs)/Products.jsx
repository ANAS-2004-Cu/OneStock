...
const ProductList = () => {
  ...
  const fetchPagedProducts = async (cursorArg = null, requestVersion = requestVersionRef.current) => {
  ...
  const fetchSearchProducts = async (cursorArg = null, requestVersion = requestVersionRef.current) => {
  ...
  const loadProducts = async (reset = false, requestVersion = requestVersionRef.current) => {
    const activeRequestVersion = requestVersion ?? requestVersionRef.current;
    ...
    try {
      ...
      if (isCuratedMode) {
        ...
        await fetchCuratedProducts(cursorToUse, activeRequestVersion);
        return;
      }
      ...
      if (isSearchMode) {
        ...
        await fetchSearchProducts(reset ? null : searchCursor, activeRequestVersion);
        return;
      }
      ...
      if (serverSideFiltersActive) {
        ...
        await fetchFilteredProducts(reset ? null : filteredCursor, activeRequestVersion);
      } else {
        if (reset) {
          setFilteredCursor(null);
        }
        await fetchPagedProducts(reset ? null : browseCursor, activeRequestVersion);
      }
    } finally {
  ...
  const handleRefresh = async () => {
    ...
    await loadProducts(true, requestVersionRef.current);
  ...
  useEffect(() => {
    ...
    loadProducts(true, currentRequestVersion);
  }, [intent]);
  ...
  const handleLoadMore = () => {
    if (hasMore && !isRefreshing && !isLoading) {
      loadProducts(false, requestVersionRef.current);
    }
  };
