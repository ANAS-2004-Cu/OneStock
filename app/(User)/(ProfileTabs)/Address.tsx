...
import AddressModal from '../../../Components/UserComponents/AddressModal';
...
const formatAddresses = (userAddresses: any[] = []): UserAddress[] => (
  userAddresses
...
const sortAddresses = (list: UserAddress[]): UserAddress[] =>
  [...list].sort((a, b) => {
...
const Address = () => {
  ...
  const [addresses, setAddresses] = useState<UserAddress[]>(() => formatAddresses(user?.Addresses || []));
  ...
  const isLoadingAddresses = loading;

  const updateStoreAddresses = useCallback((next: UserAddress[]) => {
    if (!userRef.current) {
      return;
    }
    const updatedUser = {
  ...
  const persistAddresses = useCallback(
    async (next: UserAddress[], successMessage: string) => {
  ...
  const handleSubmitAddress = async (formData: UserAddress) => {
